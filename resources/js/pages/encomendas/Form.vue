...
const isEditing = !!props.encomenda;
const titulo = computed(() => {
    const t = props.tipo === 'cliente' ? 'Cliente' : 'Fornecedor';

    return isEditing ? `Editar Encomenda — ${t}` : `Nova Encomenda — ${t}`;
});

const entidades = computed(() => props.tipo === 'cliente' ? props.clientes : props.fornecedores);

const form = useForm({
    entidade_id: props.encomenda?.entidade_id?.toString() ?? '',
    estado:      props.encomenda?.estado ?? 'rascunho',
    observacoes: props.encomenda?.observacoes ?? '',
});

const linhas = reactive(
    props.encomenda?.linhas.map(l => ({
...
const indexRoute = computed(() => props.tipo === 'cliente' ? 'encomendas.index' : 'encomendas.fornecedor.index');
...
function submit() {
    ...
    if (isEditing) {
        form.put(route('encomendas.update', props.encomenda!.id));
    } else {
        ...
        <div class="p-6 max-w-5xl mx-auto space-y-6">
            ...
            <form @submit.prevent="submit" class="space-y-6">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div class="space-y-1">
                        <Label>{{ tipo === 'cliente' ? 'Cliente' : 'Fornecedor' }} *</Label>
