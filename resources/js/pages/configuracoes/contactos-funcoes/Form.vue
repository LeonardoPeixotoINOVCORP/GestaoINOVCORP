...
const props = defineProps<{
    funcao?: { id: number; nome: string; ativo: boolean };
}>()

const isEditing = !!props.funcao;
...
function submit() {
    if (isEditing) {
        ...
        form.post(route('configuracoes.contactos-funcoes.store'));
    }
}
</script>
...
<template>
    <AppLayout>
        <Head :title="isEditing ? 'Editar Função' : 'Nova Função'" />
        <div class="p-6 max-w-lg mx-auto space-y-6">
            <div class="flex items-center justify-between">
                <h1 class="text-2xl font-semibold">{{ isEditing ? 'Editar Função' : 'Nova Função' }}</h1>
                <Link :href="route('configuracoes.contactos-funcoes.index')">
